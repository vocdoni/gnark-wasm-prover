...
import (
	...
	"github.com/vocdoni/gnark-wasm-prover/frontend/circuitdefer"
	"github.com/vocdoni/gnark-wasm-prover/frontend/internal/expr"
	"github.com/vocdoni/gnark-wasm-prover/schema"
	"github.com/vocdoni/gnark-wasm-prover/utils"
	"github.com/vocdoni/gnark-wasm-prover/utils/kvstore"

	"github.com/vocdoni/gnark-wasm-prover/constraint/solver"
	bn254r1cs "github.com/vocdoni/gnark-wasm-prover/csbn254"
)
...
func (builder *builder) addPlonkConstraint(c sparseR1C, debug ...constraint.DebugInfo) {
	if !c.qM.IsZero() && (c.xa == 0 || c.xb == 0) {
		...
		fmt.Println("adding a plonk constraint with qM set but xa or xb == 0 (wire 0)")
	}
	...
	if cID, ok := builder.mAddConstraints[h]; ok {
		...
		if c := builder.cs.GetConstraint(cID); c != nil {
			...
			if (a.WireID() != c.L.WireID()) || (b.WireID() != c.R.WireID()) {
				...
				fmt.Println("mAddConstraints entry doesn't match key")
	...
	if cID, ok := builder.mMulConstraints[h]; ok {
		...
		if c := builder.cs.GetConstraint(cID); c != nil {
			...
			if (a.WireID() != c.L.WireID()) || (b.WireID() != c.R.WireID()) {
				...
				// map with a key that don't match the entries.
				fmt.Println("mMulConstraints entry doesn't match key")
				return expr.Term{}, false
			}
